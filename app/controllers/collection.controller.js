...
const getList = async(req, res) => {
    ...
    const attributesQuery = attributes ?
        attributes.split(',') :
        ['id', 'name', 'description', 'websiteId', 'categoryId', 'status', 'createdAt', 'updatedAt'];
    ...
    Collection.findOne({
            ...
            include: [{
                ...
                },
                {
                    model: Product,
                    required: true,
                    attributes: ['id', 'name', 'price', 'isSale', 'negotiablePrice', 'images', 'url'],
                    include: [{
                        model: Category,
                        required: true,
...
const create = async(req, res) => {
    ...
    Collection.create({
        ...
        .then((collection) => {
            const collectionProductsAdd = data.collectionProducts.filter((item) => item.flag === 'add');
...
const updateRecord = async(req, res) => {
    const { id } = req.params;
    const { name, description, websiteId, categoryId, status, collectionProducts } = req.body;
    ...
    Collection.update({
        ...
        .then((collection) => {
            const collectionProductsAdd = collectionProducts.filter((item) => item.flag === 'add');
            ...
            CollectionProduct.destroy({
                where: {
                    productId: {
                        [Op.in]: collectionProductsDelete }
                }
            });
