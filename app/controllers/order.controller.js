...
const updateRecord = async(req, res) => {
    const { id } = req.params;
    const { name, phone, email, address, totalPrice, websiteId, paymentMethodId, description, status, productOrders } = req.body;
    ...
    Order.update({
        ...
        .then((order) => {
            const productOrdersAdd = productOrders.filter((item) => item.flag === 'add');
            const productOrdersCreate = productOrdersAdd.map((item) => {
                return {
                    ...item,
                    orderId: order.id
            ...
            const productOrdersUpdate = productOrders.filter((item) => item.flag === 'update');
